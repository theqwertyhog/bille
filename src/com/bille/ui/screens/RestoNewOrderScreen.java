...
public class RestoNewOrderScreen extends QFrame {

	public RestoNewOrderScreen(QWidget parent) {
		super(parent);
	...
	private void doSignals() {
		...
		placeBtn.clicked.connect(this, "placeOrder()");
	...
	private void placeOrder() {
		...
		if (order.getOrderType() == RestoOrderType.DINE_IN) {
			try {
				int ret = order.createNew();

				if (ret != 0) {
					QMessageBox.critical(this, "Error",
							order.getErrorMessage(ret));
					return;
				}
			} catch (BilleException e) {
				QMessageBox.critical(this, "Error", e.getMessage());
			...
			boolean ret = RestoBillScreen.process(this, order);
		...
		qtyBox.setValue(1);
		customerBox.setText("");
		tableNoBox.setValue(0);
		commentsBox.setText("");

		order = new RestoOrder();
		QMessageBox.information(this, "Success", "Order placed");
		new Application().showHomeScreen();
	}
	...
	private void initFormWid() {
		...
		formLay.addWidget(placeBtn, 2, 0, 1, 3, new Alignment(
				AlignmentFlag.AlignCenter));
	...
	private LongButton placeBtn = new LongButton(formWid, "Place",
			GradientColor.BLUE);
}
